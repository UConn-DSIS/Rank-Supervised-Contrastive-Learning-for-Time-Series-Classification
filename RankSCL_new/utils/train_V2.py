...
from loss.Ranking_loss import Ranking_loss_1,Ranking_loss_2
...
def train_encoder(dataset,train_loader,model_name,device,logger,train,seed,epochs_up,M,lr=1e-5,weight_decay=1e-8,batchsize=128,distance='EU'):
    ...
    loss_list = []
    best_loss = inf
    save_dir = '/home/qir21001/AAAI2022/result_model_V2/'+dataset+'_'+model_name+'_'+str(batchsize)+'_'+str(M)+'_'+str(lr)+'_'+str(epochs_up)+'_'+str(seed)
    ...
    for epoch in range(epochs_up):
        ...
        for step,(data,labels) in enumerate(train_loader,start=0):
            ...
            nclasses = unique_labels.shape[0]
            if(nclasses == 1):
                break
            ...
            for label in unique_labels:
                class_data = out_norm[labels == label]
            
                class_aug_data = pos_aug(class_data,M-class_data.shape[0],M).to(device)
               
                data_dict[label.item()] = class_aug_data
            
            loss = Ranking_loss_1(data_dict,nclasses,M,distance,device)
