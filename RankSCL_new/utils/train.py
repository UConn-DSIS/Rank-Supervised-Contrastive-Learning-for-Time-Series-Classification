...
from loss.Ranking_loss import Ranking_loss,Ranking_loss_1
...
def train_encoder(dataset,train_loader,model_name,device,logger,train,train_labels,nclasses,seed,epochs_up,loss,aug_positives=5,lr=1e-5,weight_decay=1e-8,batchsize=128,distance='EU'):
    ...
    loss_list = []
    best_loss = inf
    save_dir = '/home/qir21001/AAAI2022/result_model/'+dataset+'_'+model_name+'_'+str(batchsize)+'_'+str(aug_positives)+'_'+str(lr)+'_'+str(epochs_up)+'_'+str(seed)
    ...
    for epoch in range(epochs_up):
        ...
        for step,(data,labels) in enumerate(train_loader,start=0):
            ...
            if(model_name == 'CNN'):
                if(data.ndim==2):
                    out= model(data.unsqueeze(1).float().to(device))
                else: 
            ...
            if(model_name =='LSTM'):
                ...
                train = data.permute(0,2,1)
              
                out = model(train.to(device))
            
            pos = generate_pos(out,labels)
            
            pos_all,label = aug_data(pos,labels,aug_positives) 
            pos_all[:out.shape[0],:] = F.normalize(out,dim=1)
            data_all = pos_all
            if loss == 'rank':
                loss = Ranking_loss(data_all,label,distance,device)
                loss.backward(retain_graph=False)
            elif loss == 'ce':
                criterion = torch.nn.CrossEntropyLoss() 
                loss = criterion(torch.tensor(data_all,requires_grad=True),torch.tensor(label,dtype=torch.long))
